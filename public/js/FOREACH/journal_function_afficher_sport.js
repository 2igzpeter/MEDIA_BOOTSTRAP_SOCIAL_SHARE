...
function afficherSport(tableauDeFilms) {
    tableauDeFilms.forEach(function (news) {
      ...
      var txt_into_a_4 = document.createTextNode('5');
       ...
       function myFunction_snackbar_sport() {


        $(function(){

          $('#p_economie').text("Catégorie: Sport");
          $('#p_txt_catégorie').text("Catégorie: ");
          $('#p_catégorie').text( "Sport");

        })
}myFunction_snackbar_sport()
    ...
    $.get("http://localhost:8080/filterSport", function (response, error) {
       
        console.log(error);
        console.log(response);
     afficherSport(response);
});
}