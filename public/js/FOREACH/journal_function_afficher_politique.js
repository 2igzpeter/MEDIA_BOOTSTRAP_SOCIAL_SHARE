...
function afficherPolitique(tableauDeFilms) {
    tableauDeFilms.forEach(function (journalUtilisateur) {
     ...
     function myFunction_snackbar_politique() {
...
$(function(){
  ...
  $('#p_economie').text("Catégorie: Politique");
    $('#p_txt_catégorie').text("Catégorie: ");

    $('#p_catégorie').text( "Politique");

})
        }

myFunction_snackbar_politique();



    $.get("http://localhost:8080/filterPolitique", function (response, error) {
       
        console.log(error);
        console.log(response);
     afficherPolitique(response);
});
}